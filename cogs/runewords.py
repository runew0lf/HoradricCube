...
with open("uniques.json", "r") as read_file:
    uniques_data = json.load(read_file)

uniques_array = []
for word in uniques_data:
    uniques_array.append(word['name'])
...
class Diablo:
    ...
    async def runeword(self, ctx, *, rune):
        ...
        matches = difflib.get_close_matches(
            rune, runeword_array, 1, cutoff=0.4)[0]
        for item in data['collection1']:
            if item['runeword-name']['text'] == matches:
                embed = discord.Embed(
                    title=f"__{matches}__", description="", color=ctx.me.color)
                versionlist = [item['runeword-1.11'],
                               item['runeword-1.10'], item['runeword-1.09']]
                versionnumber = ""
                ...
                sockets = []
                split_runes = re.findall(
                    '[A-Z][^A-Z]*', item['runeword-runes'])
                for i in range(0, len(split_runes)):
                    split_runes[i] = self.lookup_emoji(
                        f"28px{split_runes[i]}_Rune")
                ...
                embed.add_field(name="Versions",
                                value=versionnumber, inline=True)
                embed.add_field(name="Info", value=info, inline=False)
                embed.add_field(name="Properties",
                                value=properties, inline=False)
        ...
        for item in runes_data:
            if item['name'].lower() == rune.lower():
                embed = discord.Embed(title=item['name'],
                                      description=self.lookup_emoji(
                                          f"28px{item['name']}_Rune"),
                                      color=ctx.me.color)
    ...
    async def item_set(self, ctx, *, item_set):
        ...
        """
        matches = difflib.get_close_matches(
            item_set, sets_array, 1, cutoff=0.4)[0]
    ...
    async def unique(self, ctx, *, item_set):
        """
        Displays uniques information
        """
        matches = difflib.get_close_matches(
            item_set, uniques_array, 1, cutoff=0.4)[0]
